...
const recordingFailedMessage = (message: string): string =>
	...
	(set, get) => ({
		...
		startRecording: async () => {
			...
			try {
				...
				proc.on("error", (error) => {
					...
					set({
						status: "idle",
						error: recordingFailedMessage(
							error instanceof Error ? error.message : "SOX process error",
						),
					});
				});
				...
				proc.on("exit", (code, signal) => {
					...
					if (currentState.status === "recording" && code !== 0) {
						...
						set({
							status: "idle",
							error: recordingFailedMessage(
								`SOX exited unexpectedly (code=${code}, signal=${signal ?? "null"})`,
							),
						});
					}
				});
				...
				set({
					status: "idle",
					error: recordingFailedMessage(
						(error as Error)?.message || "Failed to start recording",
					),
				});
				const current = get();
				current._cleanupSoxProcess();
		...
		try {
			...
			set({
				status: "idle",
				error: recordingFailedMessage(
					(error as Error)?.message || "Failed to stop recording",
				),
			});
			const current = get();
			current._cleanupSoxProcess();
