...
export function renderTranscribeSpinner(seed = 0): string {
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    if (frame === 1) { // right slash: left low to right high /
      baseHeight += normalizedI * tiltRange;
    } else if (frame === 3) { // left slash: left high to right low \
      baseHeight += (1 - normalizedI) * tiltRange;
    } else if (frame === 0) { // vertical: uniform full height |
      baseHeight = height;
    } else if (frame === 2) { // horizontal: low uniform bars
